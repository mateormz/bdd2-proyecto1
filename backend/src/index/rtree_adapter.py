...
from core.schema import Schema, Kind  # opcional para coercear filas
...
class RTreeAdapter:
    ...
    def add(
        ...
        if label_field is None:
            label_str = str(row.get("id", ""))
        else:
            label_str = str(row.get(label_field, ""))
    ...
    def remove_by_label(self, label: Any) -> int:
        ...
        for _id in list(ids):
            ...
            try:
                self.meta["label_to_ids"][label_str].remove(_id)
            except ValueError:
                pass
